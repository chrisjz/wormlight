...
- `npm run gpu:parity` — checks the GPU brain against the CPU reference in headless Chrome (PLAN §7.2), through `parity.html`, which the dev server serves and the build leaves out; `npm run dev` then `/parity.html` shows the same checks in any browser, which is how the Safari check is made. CI runs it on SwiftShader.
