...
- `uv run --project tools/reference python tools/reference/ni_reference.py` (and `cook_reference.py`) — regenerate the port and production check goldens in `tests/fixtures/`. The checks in `tests/` fail when a golden is stale: its script, its pinned inputs or Cook's wiring changed since it was made.
