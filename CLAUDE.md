...
- `npm run visual:capture` then `npm run visual:compare` — capture the fixed views from `dist/` in headless Chrome and compare them with `tests/visual/baseline/` (build first). Baselines come from CI's lavapipe captures (the `visual` artifact), not from a local GPU.
