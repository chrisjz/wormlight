...
- `npm run visual:capture` then `npm run visual:compare` — capture the fixed views from `dist/` in headless Chrome and compare them with `tests/visual/baseline/` (build first). Baselines come from CI's captures on SwiftShader, Chrome's software GPU, not a local GPU: take the `visual` artifact of a run on a branch of this repository (never a fork's), at the commit being baselined, and copy only its `<view>.png` files. A new view has no baseline on its first CI run, which fails its comparison for that reason alone: check its log says so, then take that run's artifact. `VISUAL_NEW_BASELINES=1` lets a local comparison pass without one. CI pins Chrome to the version puppeteer-core targets; bump both together and refresh the baselines in the same PR.
- `npm run gpu:parity` — checks the GPU against the CPU reference in headless Chrome (PLAN §7.2), the brain and the whole loop, through `parity.html`, which the dev server serves and the build leaves out; `npm run dev` then `/parity.html` shows the same checks in any browser, which is how the Safari check is made. CI runs it on SwiftShader. `-- --long` (or `/parity.html?long`) adds long-run parity, 11 to 18 minutes on an M5 Max and too long for CI.
- `npm run harness -- --checkpoint <n>` — runs checkpoint 0's crawling clause or checkpoint 1 on the CPU reference, one worker per core (seconds on 18 cores), and regenerates that checkpoint's section of `VALIDATION.md` between its markers. Commit the code first, so the page records a clean commit: changes to Markdown alone don't count against it, but untracked files do. `--trials` and `--seconds` shorten a run for a look, and leave `VALIDATION.md` alone.
- `npm run plate:bench` — the app's frame rate and the worm's speed at several speeds, in headless Chrome on this machine's GPU (build first). Headless Chrome paces its own frames, so the 60 fps check is `/?stats=1` in a Chrome or Safari window.
