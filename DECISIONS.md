...
- **The kernel** (`src/gpu/brainShader.ts`) takes each stage in the CPU's order: curvature from the rods; the proprioceptive currents and the head switch, gated on the network's drive on the SMDs; the brain, as in milestone 2; the neuromuscular layer; and the body, whose block-tridiagonal system is solved by block cyclic reduction on 63 rows, the rods and then identity rows, in the brain's workgroup. The brain's vectors and the body's blocks share one pool of workgroup memory, since they are never needed at once, which keeps the kernel within the 16 KB limit; it binds the 8 storage buffers a stage may have.
- **What f32 can't hold.** The body's stiffest springs, the diagonals at 7 N/m against the laterals' 0.02, stretch by nanometres, and at the thin tail by less than an ångström. So each rod's position and angle are kept as a coarse part on a power-of-two grid (2⁻²⁰ m, 2⁻¹⁰ rad) and a small remainder; its cosine and sine, and the turn between segments, come from series of our own, since WGSL allows `cos`, `sin` and `atan2` far larger errors than the body can take; a spring's stretch is taken from squared lengths; and the rotation's unknown is scaled by the body's radius. On the CPU, rounding the state alone to f32 costs up to 3 times the velocities' first tolerance, and the split storage and squared stretch brought the GPU's centre velocities within about 8 × 10⁻³ of the largest.
- **Metal reassociates.** A dump of the tail's rows showed a rod's cosine off by 4.36 × 10⁻⁸, exactly f32's error in π/2: Metal's compiler folds the three-part reduction into one, as fast-math arithmetic may. It costs precision wherever cancellation is fine, as at the tail, and no WGSL setting turns it off.
- **`GpuWorld`** (`src/gpu/world.ts`) packs a CPU World's fields, switch neurons, head region, neuromuscular rows, muscle cover and body constants into the kernel's layout, and trades whole states with it; the CPU's `World` now exports and restores a `WorldState`, and takes its brain's solver settings.
- **Parity** runs the whole loop from the rest world and twenty from its closed loop, one step and one second, beside milestone 2's brain checks; long runs are `--long`, local only (below).
- **The harnesses' dev server** runs with hot reload off, since an edit made during a twenty-minute run reloaded its page and ended it.

**Decided with the maintainer before any loop results.** The body's thresholds extend §7.2's relative rule: one step, each rod's velocity within 10⁻⁴ of the body's largest, each muscle within 10⁻⁴, the same head switch; one second, the curvature profile and the centroid's travel within 10⁻², under the well-posed rule. The floors for a body at rest (10⁻⁴ segment lengths per second, 10⁻⁴ rad/s) were filled in during implementation. That rule was committed on its own before the loop first ran on the GPU.

**Changed after results: the velocities' threshold (PLAN §7.2, marked changed).** No f32 solve can reach 10⁻⁴ on this system. Its rigid motions are held only by drag, along the body some 2,000 times weaker than the dampers and in rotation weaker still, so rounding of 10⁻⁷ in the assembled blocks becomes 10⁻⁴ to 10⁻³ in the velocities; an f32 assembly from an exact state, solved in f64, reaches 6 × 10⁻³ in some states. The maintainer chose 10⁻², the one-second threshold: a wrong sign, term or index still errs by 10⁻² to 1. In putting the choice, the rotation's error was misreported as 7.6 × 10⁻⁴ relative; it was 7.6 × 10⁻⁴ rad/s, 1 to 3% of the largest rotation rate.

**Changed after results: what the one-step check grades (PLAN §7.2, marked changed).** At 10⁻², the rods' own rotation rates still failed in 7 of 21 states, up to 2.6 times the tolerance, all at the tail, where a rod 5 µm across turns a few per cent faster or slower on the GPU and moves its ends by nanometres a second. A step of iterative refinement changed nothing, so the solve isn't the cause; the fine cancellation at the tail, and Metal's reassociation, are. The maintainer chose to grade what the springs act on: each rod's two end points, its centre's velocity plus or minus its radius times its rotation, within 10⁻² of the largest, in x and in y. Centre velocities and rotation are reported, not graded.

**Changed after results: long-run parity's seeds (PLAN §7.2, marked changed).** A pilot of 20 seeds a side, as planned, found the SD of κL equivalent (p = 0.03) but couldn't show the frequency so: its spread from seed to seed, 0.0297 Hz or 18% of the mean, put the standard error of the difference (0.0094 Hz) above the ±5% margin (0.0085 Hz) by itself, though the means differed by only 1.2%. The maintainer chose to size the run for power from the pilot's spread, not its difference: 90% power at no true difference, with Welch's two one-sided tests at α = 0.05, needs a standard error of at most 0.0085 / 3.29, so 265 seeds a side, some twenty minutes on the M5 Max. The pilot's seeds are among them.

**Long-run parity on the M5 Max** (265 seeds a side, 60 s each, about 18 minutes): both statistics are equivalent. The SD of κL is 0.2147 on the CPU and 0.2149 on the GPU, a difference of 0.1% against a margin of 5% (p = 5 × 10⁻¹⁵); the frequency is 0.1652 and 0.1663 Hz, a difference of 0.7% (p = 0.003). The 20-seed pilot had given 0.2124 and 0.2131 (p = 0.03) and 0.1700 and 0.1680 Hz (p = 0.25).
...
- **The loop, one step.** All 21 states pass. The end points' velocities are within 0.45 of their tolerance in x and 0.066 in y; the muscles within 6 × 10⁻⁴ of theirs; the head switch agrees in every state; and the brain's solver takes as many iterations as the CPU's in every state. Reported: the centres' velocities at up to 0.78 of the tolerance, and the rotation at up to 2.6.
- **The loop, one second.** The 18 graded states pass, the worst curvature error at 0.004 of its threshold and the centroid's at 0.0014; t = 4.0, 11.0 and 11.5 s are ill posed (the reference against itself at 1.1 to 1.9 of the thresholds), within the quarter the check allows.
- **Speed.** The whole step runs at about 24× real time at 2.5 ms in dispatches of 67 steps (6.8 ms each) and about 20× in dispatches of 7: more than twice the 10× target, which PLAN sets for the full step.

**Results on CI's SwiftShader** (Chrome 154), the brain's and the loop's short checks in 75 s: every check passes. The loop's one-step end points are within 0.38 of their tolerance in x and 0.04 in y, the muscles within 6 × 10⁻⁴, the switch agrees and the iteration counts match; the 18 graded one-second states pass, the curvature's worst at 0.002 of its threshold; the same three states are ill posed. The rods' own rotation, reported, is within 0.13 of the 10⁻² tolerance there, against 2.6 on Metal: SwiftShader doesn't reassociate as Metal's compiler does, which bears out the cause above. Long runs aren't run on CI.

**Status.** Milestone 3's first part is done: the loop runs on the GPU and matches the CPU reference, the full step clears the 10× target, and long-run parity has run. The behavioural harness with checkpoints 0 and 1, and the plate view, come next; milestone 3's Safari check, which now includes milestone 2's, comes with them.
