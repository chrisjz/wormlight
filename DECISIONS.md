...
- **Visual regression CI,** ported from Universe Atlas's:
  - The `visual` job serves the build, runs it in headless Chrome with WebGPU on SwiftShader, the software Vulkan that ships with Chrome, and captures four fixed views: the whole graph, the head, AVAL selected and VB6 selected. Universe's job installs Mesa's lavapipe for this, and so did the first version of this one, but the capture log showed Chrome choosing its own SwiftShader, which is better: pinning Chrome pins it too, where Mesa would float with the runner image. So the job no longer installs Mesa.
