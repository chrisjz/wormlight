...
| **R** Class-level fitting (research track) | Fallback 3: class-level gains, resting offsets and rectification in the motor circuit, tuned by §7.3's procedure, with every primary null tuned the same way and on the same budget                                      | A proposal you approve before it starts, fixing its parameters, the budget raise and its evaluation budget; it ends when checkpoint 1 reaches partial or the budget is spent |
...
**Milestone 0c's outcome (2026-09-26): no-go on crawling.** The loop is built and tested, but no setting of the planned model crawls, and neither fallback 1 nor 2 changes that (DECISIONS.md). You chose fallback 4: 0c closes as an honest partial, the ledger and the app say crawling does not yet emerge, and milestone 1 comes next. Class-level fitting becomes research track R, scheduled with you and bounded in advance. The calibration procedure (§7.3) and checkpoint 1's formal run wait for R, since no draw of the planned model comes near the targets. Until R produces a crawling worm, checkpoint 1 is reported as a fail and checkpoints 2 to 6, which need forward crawling, as not reached; checkpoint 0's silenced-network clause still runs.
...
1. **A delayed proprioceptive loop.** Wen et al. measured delays of about 80 ms per region, with ~300 ms as an upper bound. Kim et al. 2025's unfitted base model sustained locomotion once it had a delayed closed loop of about 0.5–0.6 s. _Tested at the go/no-go: delays of 80, 300 and 550 ms changed nothing, and Kim et al.'s own mechanism, which turned out to feed back the network's delayed activity rather than the body's, gave no crawling on Wormlight's network and body._
2. **Bistable B-type neurons,** as in Boyle, Berri & Cohen 2012, keeping Wen's coupling direction. _Tested: no effect._
3. **Calibrated class-level gains** in the motor circuit (level 1, counted against the budget). _Research track R (§9)._
...
| Kunert-Graf et al. 2017                                                              | The model's published parameter values                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
| Kim et al. 2025, arXiv 2504.18073 (modWorm; code at `shlizee/modWorm`, BSD-3-Clause) | The closest precedent: the same neural model on 279 neurons, with muscles and a body. Its neurons and synapses are not fitted, but the stimulus amplitudes that start each behaviour were chosen by "maximizing locomotion distance", and its body is an eel-like rod in a fluid of 10 mPa·s, not on agar. A pulse into sensory neurons (PLM forward; ALM and AVM backward) is held for about 1.2 s and then switched off, and a feedback delayed by 0.5–0.6 s sustains the movement. In their code that feedback is the network's own voltages projected onto the patterns the muscle map can see, pinv(M)·M·(V − V_th), added to the voltages; the body state is never read. A later genetic-algorithm tuning of 5,146 synapse scales was optional, and it broke backward locomotion. Its Cook-based variant fitted postures better than the Varshney base. On Wormlight's Cook network and agar body the mechanism gives no crawling (DECISIONS.md, 2026-09-26) |
...
A literature sweep at the go/no-go found no published model that makes the whole connectome crawl on agar with anatomy's weights and without fitting, and Wormlight doesn't yet either (DECISIONS.md, 2026-09-26). The circuits above that crawl on agar were fitted, and most were also reduced to repeating units; Kim et al.'s unfitted network moves in a fluid, from stimuli chosen for distance.
