...
On the GPU the whole step runs in one workgroup of 256 invocations, each holding the state of one or two neurons in registers, within the default limits: it binds 7 of the 8 storage buffers a stage may have and uses 8 of the 16 KB of workgroup memory. A dispatch takes any number of steps, with only barriers between them (`src/gpu/brainShader.ts`).
...
| **GPU parity, the body, muscles and head switch** (set 2026-09-26, before any loop parity results, DECISIONS.md). From milestone 3, the same states carry the body, the muscles and the head switch, and both sides run the whole loop. **Changed after results (2026-09-26, DECISIONS.md), twice:** the velocities' threshold from 10⁻⁴, which f32 can't reach on this system, to 10⁻²; and what is graded, from each rod's centre and rotation to its two end points, where rotation counts by the rod's radius. Centre velocities and rotation are reported | One step: the velocities of each rod's two end points within 10⁻² of the body's largest, normwise and separately for x and y, with a floor of 10⁻⁴ segment lengths per second for a body at rest; \|ΔA\| ≤ 10⁻⁴ for every muscle; the same head-switch state. One second: the RMS error of κL over the interior rods ≤ 10⁻² × max(RMS κL, 1), and the centroid's displacement within 10⁻² of itself (floor 0.01 body lengths), graded under the one-second row's well-posed rule |
| **GPU parity, long runs.** Seeds 1 upwards, 60 s each, from milestone 3, when the body is on the GPU. **Changed after results (2026-09-26, DECISIONS.md):** 265 seeds a side, not 20, sized for 90% power from a 20-seed pilot's spread                                                                                                                                                                                                                                                                                                                        | Crawling frequency and speed equivalent within ±5% of the CPU's mean (Welch's two one-sided tests, α = 0.05). While checkpoint 1 is below partial, the mid-body curvature's frequency and its standard deviation of κL instead, as `scripts/experiments/go-no-go/loop.ts` measures them, by the same test (set 2026-09-26, before any parity results, DECISIONS.md)                                                                                                              |
...
- from milestone 3 the same states are also whole worlds, and both sides run the whole loop, the body included (`src/gpu/loopParity.ts`);
- long runs (`npm run gpu:parity -- --long`, or `/parity.html?long`) take some twenty minutes on a laptop's GPU and would take hours on SwiftShader, so they run locally and at the Safari check, not in CI;
