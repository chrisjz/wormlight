...
| **GPU parity, long runs.** 20 seeds per side, 60 s each, from milestone 3, when the body is on the GPU                                                                                                                                                                                                                                                              | Crawling frequency and speed equivalent within ±5% (two one-sided tests). While checkpoint 1 is below partial, the mid-body curvature's frequency and its standard deviation of κL instead, by the same test (set 2026-09-26, before any results, DECISIONS.md)                                                                |
...
- the random numbers are checked first: the GPU's hashes and uniforms must equal the CPU's exactly, and each Gaussian must lie within the error WGSL allows `log`, `sqrt` and `cos`, which the one-step check adds to its tolerance;
...
| 2                     | The neural model on the GPU, with the parity CI job                                                                                                                                                                      | Parity passes on CI's software GPU and on the Mac; a Safari check; the brain step's speed measured against the 10× target. Long-run parity and the full step's speed moved to milestone 3 (2026-09-26) |
| 3                     | The body on the GPU and the plate view                                                                                                                                                                                   | Checkpoint 1 in the harness, a fail until track R succeeds; long-run GPU parity; 60 fps real time on an M-series Mac in Chrome and Safari; the full step's speed against the 10× target                |
