...
// which the parity check bounds (parity.ts).
...
// Box–Muller. WGSL bounds cos's error only on [−π, π], so cos(2π u) is taken as −cos(π (2u − 1)), whose
// argument is computed exactly but for the rounding of π and of one product.
fn gaussian_from(h1: u32, h2: u32) -> f32 {
  return sqrt(-2.0 * log(unit(h1))) * -cos(3.1415927 * (2.0 * unit(h2) - 1.0));
}
