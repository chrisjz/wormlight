...
// parameters (PLAN §6.2), heading where its seed says, by a hash no noise draw, head-switch draw, AWC-ON draw
// or trial's posture draw uses, and adapted to the odour where it lies.
...
export function appWorld(data: WormlightData, seed: number, odour?: Odour): World {
  const world = new World(data, provisionalParams(), { seed, heading: startingHeading(seed), odour });
