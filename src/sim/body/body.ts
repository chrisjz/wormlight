...
// u_i = (cos θ_i, sin θ_i), and its ventral point the centre minus that. Rod 0 is the head; a straight body
// lies along t = (sin θ, −cos θ), from head to tail.
//
// Each step is linearly implicit Euler. The velocities q̇ of the rods (x, y, θ) solve
// (Ξ − B − dt·K) q̇ = G, where Ξ is the drag, B the dampers, K the Jacobian of the elastic forces G, and
// then q advances by dt·q̇. The matrix is symmetric and block tridiagonal (49 blocks of 3 × 3), because
// each element joins neighbouring rods; the CPU solves it by block elimination.
...
export function boyleBody(radii?: Float64Array): BodyParams {
  ...
  return {
    ...
    efficacy: Float64Array.from({ length: segments }, (_, m) =>
      m === 0 ? (p.muscleEfficacy.value * 2) / 3 : p.muscleEfficacy.value - (0.42 * m) / segments,
    ),
...
export class Body {
  ...
  straighten(x = 0, y = 0, heading = Math.PI): void {
    ...
    // Along the body from head to tail is t = (sin θ, −cos θ), so θ = heading + π/2 points it tailwards.
  ...
  // The elastic and muscle forces on each rod, [Fx, Fy, torque] per rod, with no external force: the
  // right-hand side G. For tests.
  elasticForce(): Float64Array {
    this.assemble(0);
    const g = Float64Array.from(this.rhs);
    for (let i = 0; i < this.rods; i++) {
      g[3 * i] -= this.force[2 * i];
      g[3 * i + 1] -= this.force[2 * i + 1];
    }
    return g;
  }

  // The system matrix (Ξ − B − dt·K) as a dense row-major array, for tests.
  systemMatrix(dt: number): Float64Array {
    this.assemble(dt);
    const size = 3 * this.rods;
    const dense = new Float64Array(size * size);
    for (let i = 0; i < this.rods; i++) {
      for (let r = 0; r < 3; r++) {
        for (let c = 0; c < 3; c++) {
          dense[(3 * i + r) * size + 3 * i + c] = this.diag[9 * i + 3 * r + c];
          if (i + 1 < this.rods) {
            dense[(3 * i + r) * size + 3 * (i + 1) + c] = this.upper[9 * i + 3 * r + c];
            dense[(3 * (i + 1) + c) * size + 3 * i + r] = this.upper[9 * i + 3 * r + c];
          }
        }
      }
    }
  ...
  private assemble(dt: number): void {
    const p = this.params;
    ...
    for (let m = 0; m < p.segments; m++) {
      ...
      this.element(m, 1, -1, p.diagonalStiffness, this.restDiagonal[m], p.diagonalDamping, dt);
      this.element(m, -1, 1, p.diagonalStiffness, this.restDiagonal[m], p.diagonalDamping, dt);
    }
  }

  // Add one element joining side `sa` of rod m to side `sb` of rod m + 1: a spring of stiffness k and rest
  // length `rest`, and a damper β. Its generalized force is f·g with f = k(rest − L) and g = Dᵀn, where n
  // is the unit vector from its first point to its second and D maps the two rods' velocities to the
  // velocity of the second point relative to the first. Its share of (Ξ − B − dt·K) is
  // (β + dt·k) g gᵀ − (dt·f/L) h hᵀ − dt·f·(the change of D with each rod's angle), with h = Dᵀn⊥.
  private element(m: number, sa: number, sb: number, k: number, rest: number, beta: number, dt: number): void {
    const p = this.params;
    const a = m;
    const b = m + 1;
    const ca = this.cos[a];
    ...
    h[5] = rb * (snb * ny + cb * nx);
    const w = beta + dt * k;
    const bend = (dt * f) / length;
    const { diag, upper, rhs } = this;
    for (let r = 0; r < 3; r++) {
      rhs[3 * a + r] += f * g[r];
      rhs[3 * b + r] += f * g[r + 3];
      for (let c = 0; c < 3; c++) {
        diag[9 * a + 3 * r + c] += w * g[r] * g[c] - bend * h[r] * h[c];
        diag[9 * b + 3 * r + c] += w * g[r + 3] * g[c + 3] - bend * h[r + 3] * h[c + 3];
        upper[9 * a + 3 * r + c] += w * g[r] * g[c + 3] - bend * h[r] * h[c + 3];
      }
    }
    // The ends turn with their rods: the change of D with θ puts −dt·f·(±R u·n) on each angle's diagonal.
    diag[9 * a + 8] -= dt * f * ra * (ca * nx + sna * ny);
    diag[9 * b + 8] += dt * f * rb * (cb * nx + snb * ny);
  }
