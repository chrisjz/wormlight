...
- Kim et al. 2025 (arXiv 2504.18073; code `shlizee/modWorm`): connectome, neural dynamics, muscles and biomechanics. With its synapses unfitted, it moved forward and backward in a fluid that stands for agar after a brief pulse into sensory neurons, sustained by a ~0.5–0.6 s delayed feedback of the network's own activity rather than body sensing, with the delay chosen to match recorded curvatures. A later genetic-algorithm tuning of 5,146 synapse scale factors was optional. The delayed closed loop was the first fallback in §2.4; at milestone 0c it gave no crawling on Wormlight's network and body (DECISIONS.md, 2026-09-26).
